...
def ensure_group_defaults(chat_id: int):
    """
    ✅ Ensure group exists in DB with default settings
    """
    chat_id = int(chat_id)
...
def update_group(chat_id: int, data: dict):
    """
    💾 Update group settings in DB + cache
    """
    chat_id = int(chat_id)
    groups_col.update_one(
        {"chat_id": chat_id},
    ...
    group_settings[chat_id].update(data)
...
# GET TIME / INFO
# ---------------------------
@dp.message(Command("gettime"))
@dp.message(Command("delayinfo"))
...
async def on_startup(bot: Bot):
    ...
    print(f"📦 Groups Cached  : {len(group_settings)}")
    print(f"💎 Premium Cached : {len(premium_users)}")
    print("=" * 40 + "\n")
